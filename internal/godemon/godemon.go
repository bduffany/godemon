...
func (cfg *Config) prepare() error {
	...
	if cfg.Watch != nil && len(cfg.Watch) == 0 {
		return fmt.Errorf("watch list in config is empty")
	}
	if len(cfg.Watch) == 0 {
		cfg.Watch = []string{"."}
	}
	...
	if *cfg.UseGitignore {
		for _, w := range cfg.Watch {
			if err := addGitignorePatterns(cfg, w); err != nil {
				return fmt.Errorf("failed to add .gitignore patterns: %s", err)
			}
		}
	}
...
func (g *godemon) handleAdds(addCh chan string, done <-chan struct{}) {
	for i := 0; i < 16; i++ {
		go func() {
			for {
				var path string
